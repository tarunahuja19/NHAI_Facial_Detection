...
It uses JAX distributed initialization and GSPMD sharding to scale data parallelism across all hosts
and devices, with robust multi-host checkpointing via Orbax and clean single-rank logging.
...
class TrainConfig:
    # ── paths ──
    output_dir:       str   = "./output_vggface2_cluster"
    checkpoint_dir:   str   = "./checkpoints_vggface2_cluster"
    log_file:         str   = "./train_vggface2_cluster.log"
...
def train(cfg: TrainConfig = CFG):
    ...
    logger.info(
        f"  Total Train: {len(train_ds):,} samples "
        f"| Total Val: {len(val_ds):,} samples "
        f"({len(set(val_labels_raw))} identities)"
    )

    # Shard the training dataset across global processes (hosts)
    if jax.process_count() > 1:
        logger.info(f"Sharding training dataset: process {jax.process_index()+1}/{jax.process_count()}...")
        train_ds = train_ds.shard(num_shards=jax.process_count(), index=jax.process_index())
        logger.info(f"  Local process training dataset size: {len(train_ds):,} samples")

    steps_per_epoch = len(train_ds) // local_batch_size
