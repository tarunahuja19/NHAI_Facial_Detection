...
It uses JAX's modern GSPMD sharding API (jax.sharding) to run data-parallel
training across all available local GPUs.
...
class TrainConfig:
    # ── paths ──
    output_dir:       str   = "./output_vggface2_gpu"
    checkpoint_dir:   str   = "./checkpoints_vggface2_gpu"
    log_file:         str   = "./train_vggface2_gpu.log"

    # ── dataset ──
    dataset_name:     str   = "chronopt-research/cropped-vggface2-224"
    image_dir:        str   = ""        # Path to local folder containing extracted images (for text-only datasets like RichardErkhov/VGGFace2)
    image_size:       int   = 112
    val_fraction:     float = 0.05      # fraction of identities held out for val (e.g. ~450 out of 9131)
    num_workers:      int   = 4
    prefetch:         int   = 2

    # ── model ──
    num_classes:      int   = 9131      # Default VGGFace2 total identities (will be overwritten by dataset)
    ...
    # ── optimiser ──
    batch_size:       int   = 128        # Global batch size (split across all GPUs)
    num_epochs:       int   = 25
    warmup_epochs:    int   = 1
...
# ──────────────────────────────────────────────────────────────────────────────
#  Dataset splits with label reindexing
# ──────────────────────────────────────────────────────────────────────────────
...
def train_val_split_hf(
    ...
    val_fraction: float,
    seed: int = 42,
) -> Tuple[List[int], List[int], Dict[any, int]]:
    """Hold out `val_fraction` of identities entirely for validation and maps labels to 0..N-1."""
    if "label" in dataset.column_names:
        labels = np.array(dataset["label"])
    elif "text" in dataset.column_names:
        # Extract folder name as the label from the text column
        # e.g., "n000002/0001_01.jpg" -> "n000002"
        paths = dataset["text"]
        labels = np.array([p.split('/')[0] if '/' in p else p for p in paths])
    else:
        raise ValueError(f"Dataset does not contain 'label' or 'text' column. Columns: {dataset.column_names}")

    unique_labels = np.unique(labels)
    
    # Map labels to consecutive integers 0..N-1
    ...
    return train_indices, val_indices, label_to_idx
...
def batch_generator_hf(
    ...
    is_text_only = "text" in dataset.column_names and "image" not in dataset.column_names
    if is_text_only:
        # Pre-fetch the paths list upfront to avoid Arrow index overhead in the training loop
        paths = dataset["text"]
        ...
        for start in range(0, max(end, 0), batch_size):
            ...
            if is_text_only:
                for idx in batch_idx:
                    try:
                        path_rel = paths[idx]
                        label_raw = path_rel.split('/')[0] if '/' in path_rel else path_rel
                        if not image_dir:
                            raise ValueError("Dataset contains relative paths ('text' column) but 'image_dir' was not provided in config.")
                        
                        img_path = os.path.join(image_dir, path_rel)
                        pil_img = Image.open(img_path)
                        
                        label = label_to_idx[label_raw]
                        ...
                        if consecutive_failures > max_consecutive_failures:
                            raise RuntimeError(
                                f"Too many consecutive image loading failures (exceeded {max_consecutive_failures}). "
                                f"Last error: {e}. Please check if your --image_dir is correct (currently '{image_dir}') and contains the images."
                            ) from e
                        continue
            else:
                try:
                    # Fetch batch of rows at once using HF list indexing (much faster than row-by-row)
                    batch = dataset[batch_idx]
                    for pil_img, label_raw in zip(batch["image"], batch["label"]):
                        try:
                            label = label_to_idx[label_raw]
                            ...
                            if consecutive_failures > max_consecutive_failures:
                                raise RuntimeError(
                                    f"Too many consecutive image loading failures (exceeded {max_consecutive_failures}). "
                                    f"Last error: {e}. Please check if the dataset images are valid."
                                ) from e
                            continue
                except Exception as e:
                    ...
                    if consecutive_failures > max_consecutive_failures:
                        raise RuntimeError(
                            f"Too many consecutive dataset batch retrieval failures (exceeded {max_consecutive_failures}). "
                            f"Last error: {e}."
                        ) from e
                    continue
...
def restore_checkpoint(
    ...
    state = mngr.restore(
        latest,
        args=ocp.args.PyTreeRestore(item=state, partial_restore=True),
    )
    return state, latest
...
def train(cfg: TrainConfig = CFG):
    ...
    logger.info(f"JAX devices found: {devices}")
    
    # Establish a 1D parallel mesh along the 'data' dimension for Data Parallelism
    logger.info(f"Initialising GPU 1D Mesh of size {num_devices} for multi-GPU training...")
    ...
    # ── dataset ──────────────────────────────────────────────────────────────
    logger.info(f"Loading {cfg.dataset_name} dataset from Hugging Face...")
    from datasets import load_dataset
    hf_ds = load_dataset(cfg.dataset_name, split="train")
    logger.info(f"  Total samples in dataset: {len(hf_ds):,}")

    logger.info("Splitting dataset into train/val by identities and mapping labels...")
    train_indices, val_indices, label_to_idx = train_val_split_hf(hf_ds, cfg.val_fraction, seed=42)
    
    # Dynamically overwrite the number of classes from dataset
    cfg = cfg.replace(num_classes=len(label_to_idx))
    ...
    if "label" in val_ds.column_names:
        val_labels_raw = val_ds["label"]
    else:
        val_labels_raw = [p.split('/')[0] if '/' in p else p for p in val_ds["text"]]

    logger.info(
        f"  Train: {len(train_ds):,} samples "
        f"| Val: {len(val_ds):,} samples "
        f"({len(set(val_labels_raw))} identities)"
    )

    steps_per_epoch = len(train_ds) // cfg.batch_size
    total_steps     = steps_per_epoch * cfg.num_epochs
    logger.info(
        f"  Global batch size: {cfg.batch_size} (local batch size per GPU: {cfg.batch_size // num_devices}) "
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        for step in pbar:
            ...
            # Distribute data along the batch axis across GPUs
            imgs_sharded = jax.device_put(jnp.array(imgs), data_sharding)
            lbls_sharded = jax.device_put(jnp.array(lbls), label_sharding)
        ...
        if (epoch + 1) % cfg.val_every == 0:
            val_acc = evaluate_rank1(
                val_ds, state, model, label_to_idx, cfg, logger
            )
            if val_acc > best_val:
                best_val = val_acc
